 ...
 * @class
 */
class GitHubTableParser : public BlockParser
{
public:
  /**
   * ctor
   ...
   * IsStartingLine
   *
   * If the line starts with `|`, then it is starting the table.
   *
   * @method
  ...
  AddLine(std::string& line) override
  {
    ...
    if (this->isStarted)
    {
      if (std::regex_match(line, alignRegex))
      {
        ++this->currentBlock;
        this->currentRow = 0;
        return;
      ...
      std::string segment;
      std::stringstream streamToSplit(std::string(line.begin() + 1, line.end() - 1));
