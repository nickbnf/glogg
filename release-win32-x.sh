...
if [ "$1" == "debug" ]; then
    ...
    qmake-qt4 glogg.pro -spec win32-x-g++ -r CONFIG+="debug win32 rtti no-dbus" BOOST_PATH=$BOOSTDIR
elif [ -z "$VERSION" ]; then
    echo "Building default version"
    qmake-qt4 glogg.pro -spec win32-x-g++ -r CONFIG+="release win32 rtti no-dbus" BOOST_PATH=$BOOSTDIR
else
    echo "Building version $VERSION"
    qmake-qt4 glogg.pro -spec win32-x-g++ -r CONFIG+="release win32 rtti no-dbus" BOOST_PATH=$BOOSTDIR VERSION="$VERSION"
fi
make -j3
